...
import ProductController from "./assetProduct.controller";
import {
...
export async function assetProductRouter(app: FastifyInstance): Promise<void> {
  ...
  app.post("/edit", async (request, reply) => {
    ...
    try {
      ...
      const controller = new ProductController();
      const result = await controller.editProduct(body as EditAssetProductBody);
  ...
  app.post<{ Body: DeleteAssetProductBody }>(
    "/delete",
    {
      schema: {
        body: {
  ...
  app.post<{
    ...
    async (
      request: FastifyRequest<{ Body: GetAssetProductReport }>,
      reply: FastifyReply
    ) => {
